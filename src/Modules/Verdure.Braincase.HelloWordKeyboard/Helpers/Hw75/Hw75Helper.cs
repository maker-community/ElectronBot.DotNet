...
namespace Verdure.Braincase.HelloWordKeyboard.Helpers;
...
public class Hw75Helper
{
    ...
    public async Task SyncDataToDeviceAsync(UIElement? element)
    {
        if (Hw75Helper.Instance.IsConnected)
        {
            try
            {
                ...
                var dpi = 96;//App.MainWindow.GetDpiForWindow();
    ...
    public async Task HaSwitchAsync(bool isOn)
    {
        ...
        var haSwitchModel = await localSettingsService.ReadSettingAsync<ComboxItemModel>(CommonConstants.DefaultHaSwitchNameKey);

        var haSetting = await localSettingsService.ReadSettingAsync<HaSetting>(CommonConstants.HaSettingKey);
